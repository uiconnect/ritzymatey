...
$.ajax({
  ...
  type:"POST",
  success: function (d) {
    for(var i in d.colorAvailability){
      $("[value='"+ d.colorAvailability[i] +"']").attr("checked","checked")
    }
    for(var i in d.sizeAvailability){
      $("[value='"+ d.sizeAvailability[i] +"']").attr("checked","checked")
    }
